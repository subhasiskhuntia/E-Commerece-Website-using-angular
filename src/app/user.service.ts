...
export class UserService {
  ...
  BuyTheProduct(
    ...
    options.handler = (response: any, error: any) => {
      ...
      this.saveRecord(orderDetails).subscribe(
  ...
  getOrderDetails(): Observable<OrderDetails[]> {
    return this.http.post<OrderDetails[]>(
      'http://localhost:8081/api/user/getOrderDetails',
      { username: sessionStorage.getItem('userName') }
    );
  }
  getUserDetails(): Observable<User> {
    return this.http.post<User>(
      'http://localhost:8081/api/user/getUserDetails',
      { username: sessionStorage.getItem('userName') + '' }
    );
  }
  changeUserDetails(user: User): Observable<string> {
    return this.http.post(
      'http://localhost:8081/api/user/changeUserDetails',
      user,
      { responseType: 'text' }
    );
  }
